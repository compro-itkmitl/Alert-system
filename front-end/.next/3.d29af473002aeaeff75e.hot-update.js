    ...
    _createClass(DashBoard, [{
        ...
        value: function componentDidMount() {
            ...
            firebaseFilter.on("value", function (snapshot) {
                ...
                if (__WEBPACK_IMPORTED_MODULE_5_firebase__["database"]().ref().child('Sensor').orderByChild('status').equalTo('Alert')) {
                    var Sound = document.getElementById("Sound");
                    Sound.autoplay = true;
                    Sound.load();
                }
                /*
                    check data of object only
                    for (let i = 0;i <= Sensor.length;i++) {
                        if (firebaseFilter.orderByChild('status').equalTo('Alert')) {
...
//# sourceMappingURL=3.d29af473002aeaeff75e.hot-update.js.map